...
/**
 * Explains what the entity's join policy means for the member-approval flow, so an
 * admin understands why a pending queue is (or isn't) shown. Values match the
 * backend JoinPolicy enum. Legacy "FREE" is treated as OPEN.
 */
...
> = {
  OPEN: {
    icon: DoorOpen,
    title: "Open — anyone can join",
    body: (e) => `New members join this ${e} instantly. There is nothing to approve.`,
    tone: "bg-muted/40 border-border text-muted-foreground",
  },
  APPROVAL: {
    icon: ShieldCheck,
    title: "Approval required",
    body: (e) => `Requests to join this ${e} wait for your decision. Approve or decline each one below.`,
    tone: "bg-warning/10 border-warning/30 text-foreground",
  },
  INVITE_ONLY: {
    icon: Mail,
    title: "Invite only",
    body: (e) => `People can't request to join this ${e} — add them with Invite Member.`,
    tone: "bg-primary/10 border-primary/30 text-foreground",
  },
  PAID: {
    icon: CreditCard,
    title: "Paid membership",
    body: (e) => `Members join this ${e} after paying. A request appears here only once its payment succeeds.`,
...
function normalize(policy: JoinPolicyValue): "OPEN" | "APPROVAL" | "INVITE_ONLY" | "PAID" {
  ...
  return "OPEN"; // OPEN, FREE, empty, or anything unrecognised
}

export function JoinPolicyBanner({
  joinPolicy,
  entityLabel,
}: {
  joinPolicy: JoinPolicyValue;
  entityLabel: "community" | "association";
}) {
