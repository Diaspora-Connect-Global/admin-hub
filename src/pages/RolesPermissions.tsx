...
export default function RolesPermissions() {
  ...
  const filteredAdmins = admins.filter((admin: AdminAccount) =>
    admin.email.toLowerCase().includes(adminSearchFilter.toLowerCase()) ||
    admin.adminType.toLowerCase().includes(adminSearchFilter.toLowerCase())
  );
  ...
  const handleRevokeRole = async () => {
    ...
    try {
      const result = await revokeRoleMutation({
        variables: { roleAssignmentId: selectedRoleId },
      });
      if (result.data?.revokeAdminRole?.success) {
        toast({
          title: "Role Revoked",
          description: "Role has been revoked successfully.",
        });
        await refetchAdmins();
        setRevokeRoleOpen(false);
      }
    } catch (error) {
      toast({
        title: "Error",
  ...
  return (
    <AdminLayout>
      <div className="p-6 space-y-6">
            ...
            {adminsLoading ? (
              ...
              <div className="table-container">
                <Table>
                  ...
                  <TableBody>
                    {filteredAdmins.map((admin: AdminAccount) => (
                      <TableRow
                        key={admin.id}
                            ...
                            >
                              ...
                              {admin.roles && admin.roles.length > 0 && (
                                <DropdownMenuItem
                                  ...
                                  onClick={() => {
                                    setSelectedAdmin(admin);
                                    setSelectedRoleId(admin.roles[0].id);
                                    setRevokeRoleOpen(true);
                                  }}
                                >
                                  <X className="w-4 h-4" /> Revoke Role
                                </DropdownMenuItem>
                              )}
                            </DropdownMenuContent>
                          </DropdownMenu>
              ...
              <>
                ...
                <div className="table-container overflow-x-auto">
                  <Table>
                    <TableHeader>
                      <TableRow className="border-border hover:bg-transparent">
                        <TableHead className="text-muted-foreground min-w-[180px] sticky left-0 bg-card">
                          Resource
                        </TableHead>
                        {actions.map((action) => (
                          <TableHead
                            key={action}
                            className="text-center text-muted-foreground capitalize min-w-[100px]"
                          >
                            <div className="flex flex-col items-center gap-1">
                              <span>{action}</span>
                              <Checkbox
                                checked={resources.every(
                                  (r) => permissions[r.id]?.[action]
                                )}
                                onCheckedChange={() =>
                        ...
                        <TableHead className="text-center text-muted-foreground min-w-[80px]">
                          All
                        </TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {filteredResources.map((resource) => {
                        const allEnabled = actions.every(
                          (a) => permissions[resource.id]?.[a]
                        );
                        const isRestricted =
                          resource.id === "settings" ||
                          resource.id === "audit_logs";
                          ...
                          >
                            ...
                            {actions.map((action) => (
                              <TableCell key={action} className="text-center">
                                <Checkbox
                                  checked={
                                    permissions[resource.id]?.[action] || false
                                  }
                                  onCheckedChange={() =>
                            ...
                            <TableCell className="text-center">
                              <Checkbox
                                checked={allEnabled}
                                onCheckedChange={() =>
                                  toggleAllForResource(resource.id)
                                }
        ...
        {/* Revoke Role Dialog */}
        <Dialog open={revokeRoleOpen} onOpenChange={setRevokeRoleOpen}>
          <DialogContent className="bg-card border-border">
            <DialogHeader>
              <DialogTitle>Revoke Role</DialogTitle>
              <DialogDescription>
                Are you sure you want to revoke this role from{" "}
                {selectedAdmin?.email}?
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
              <Button
                variant="outline"
                onClick={() => setRevokeRoleOpen(false)}
              >
                Cancel
              </Button>
