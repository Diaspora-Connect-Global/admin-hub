/** Maps admin UI country labels (same list as Communities / CommunityDetail) to ISO 3166-1 alpha-2 for API payloads. */
export const COUNTRY_LABEL_TO_ISO2: Record<string, string> = {
  "Afghanistan": "AF",
  "Albania": "AL",
  "Algeria": "DZ",
  "Andorra": "AD",
...
function labelToIso2(label: string): string {
  ...
  return COUNTRY_LABEL_TO_ISO2[t] ?? t;
}

/** Convert MultiSelect country labels to ISO codes for create/update community APIs. */
export function countriesServedLabelsToIso2(labels: string[]): string[] {
  return labels.map(labelToIso2).filter(Boolean);
}

export function singleCountryLabelToIso2(label: string | null | undefined): string | null {
